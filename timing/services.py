"""
Data source for the dashboard: a single state.json file published to
GitHub Pages by the "F1 Live Poller" Action in this repo (see
.github/workflows/poll.yml and poller.py). That Action does the actual
talking to F1's live timing feed; this module just fetches its output on
a short cache, the same role OpenF1 used to play.

Set settings.GITHUB_STATE_URL to your own published state.json before
...
def _fetch_state() -> dict:
    cached = cache.get("github_state")
    if cached is not None:
        return cached

    url = settings.GITHUB_STATE_URL
    if not url:
        return {"ok": False, "reason": "GITHUB_STATE_URL is not set in settings.py"}
    ...
    try:
        ...
        # time rather than blanking out on a hiccup or a GitHub Pages 404.
        data = {"ok": False, "reason": f"Could not fetch state.json: {exc}"}

    cache.set("github_state", data, settings.GITHUB_STATE_CACHE_SECONDS)
    return data
